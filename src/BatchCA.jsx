...
// Nettoyage intelligent du nom des colonnes
const normalize = (s = "") =>
  s
    .toString()
    .normalize("NFKD")
    .replace(/\s+/g, " ")         // remplace les tab, espaces multiples
...
export default function BatchCA() {
  ...
  const onFile = async (e) => {
    ...
    try {
      ...
      const columnsRaw = Object.keys(json[0] || {});
      const normalizedMap = Object.fromEntries(
        columnsRaw.map(c => [normalize(c), c])
      );

      // Recherche intelligente
      const candidates = ["SIREN", "NUMERO SIREN", "ID SIREN"];

      let found = null;
      for (const c of candidates) {
        if (normalizedMap[c]) {
          found = normalizedMap[c];
          break;
        }
      }

      // Si rien trouvé → on prend "SIREN" même si espace
      if (!found) {
        found = columnsRaw.find(c => normalize(c).includes("SIREN")) || columnsRaw[0];
      }
      ...
      setLog(
        `Feuille "${first}" chargée (${json.length} lignes). Colonne détectée pour le SIREN : "${found}".`
      );
  ...
  const canRun = useMemo(() => !!rows.length && !!colName && !working, [
    rows,
    colName,
    working,
  ]);
      ...
      <div className="flex items-center gap-3">
        ...
        {rows.length > 0 && (
          <div className="text-xs text-gray-600">
            {rows.length} lignes chargées — Colonne SIREN détectée : <b>{colName}</b>
          </div>
        )}
      </div>
